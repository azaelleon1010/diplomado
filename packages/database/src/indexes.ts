 ...
 */
export const baseIndexSpecs: IndexSpec[] = [
  {
...
export const exampleIndexesForFutureModules: IndexSpec[] = [
  {
...
export async function ensureIndexes(models: Model<unknown>[]): Promise<void> {
  for (const model of models) {
    try {
      ...
      logger.error({ err: (err as Error).message, collection: model.collection.name }, 'index sync failed');
      throw err;
    }
  }
}

export function logIndexCatalog(): void {
  logger.info({ base: baseIndexSpecs, examples: exampleIndexesForFutureModules }, 'index catalog');
}
